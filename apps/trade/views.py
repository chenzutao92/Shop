...
from .serializers import ShopCartSerializers, ShopCartDetailSerialixers, OrderDetailSerializer, OrderSerializer
...
class ShoppingCartViewset(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == "list":
            return ShopCartDetailSerialixers
        else:
...
class AlipayView(APIView):
    def get(self, request):
        ...
        if verify_re is True:
            ...
            exised_orders = OrderInfo.objects.filter(order_sn=order_sn)
            for exised_order in exised_orders:
                exised_order.pay_status = trade_status
                exised_order.trade_no = trade_no
                exised_order.pay_time = datetime.now()
